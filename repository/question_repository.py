...
class QuestionRepository:
    ...
    def set_answer_checked_with_no_expert_answer(self,mysql_db_manager,new_question):
        query = "UPDATE question SET answer_checking_processed = '1',has_answer='0' WHERE url='" + escape_string(str(new_question.url)) + "' " \
                "and server_id='"+str(server_id)+"' "
        mysql_db_manager.excute_query(query)

    def set_answer_checked_with_expert_answer(self, mysql_db_manager, new_question):
        query = "UPDATE question SET answer_checking_processed = '1',has_answer='1' WHERE url='" + escape_string(str(new_question.url)) + "' " \
                                     "and server_id='" + str(server_id) + "' "
        mysql_db_manager.excute_query(query)
    ...
    def set_broken(self, mysql_db_manager, question,reason):
        query = "UPDATE question SET is_processed = '2' ,state = '"+str(reason)+"' WHERE url='" + escape_string(str(question.url)) + "' " \
        " and server_id='" + str(server_id) + "' "
        mysql_db_manager.excute_query(query)
    ...
    def set_page_html(self, mysql_db_manager, question, html):
        query = "UPDATE question SET page_html = '" + escape_string(str(html)) + "' WHERE url='" + escape_string(str(question.url)) + "' " \
              " and server_id='" + str(server_id) + "' "
        mysql_db_manager.excute_query(query)

    def set_question_html(self, mysql_db_manager, question, question_html):
        query = "UPDATE question SET question_html = '" + escape_string(str(question_html)) + "' WHERE url='" + escape_string(
            str(question.url)) + "' " \
        " and server_id='" + str(server_id) + "' "
        mysql_db_manager.excute_query(query)
    ...
    def set_question_info(self,mysql_db_manager,question,answer,question_html,page_html,reason):
        query = "UPDATE question SET question_html = '" + escape_string(str(question_html)) + "' " \
            ",answer_html='" + escape_string(str(answer)) + "' " \
